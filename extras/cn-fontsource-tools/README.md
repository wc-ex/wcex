...
- 本项目期望尽可能提升中文字库加载速度，以达成中文字库在浏览器的可用性。
- 本工具可直接发布到NPM，在线网站可直接通过CDN访问，离线需下载相应Npm包。
- 发布字体包请使用开源免费字体，如添加商用授权字体库，务必添加出处、授权信息等相关信息。
- 使用时尊重版权和原创，必要时购买相应授权。字体创作也和开发一样，需要大量心血的付出和成本，健康的发展才能有更多的优秀作品。

## 说明
  1. 字体将生成2种包，第一种为切割的分段字体，适用于不确定显示内容（如用户输入内容，文章等）
  主要计划更新:
  1. 完善原有字体生成工具
...
## 技术和方案
  - 字体制作工具将生成两种字体包：split 和 single。
  - split 包根据组合进行分割，对应不确定内容或者大块内容（如: 文章等）
  - single 包为单字包，对应确定内容的少量文字，加载速度超快。
### 
